...
void Win_GParted::set_valid_operations()
{
	    ...
	    ! m_devices[m_current_device]->is_partition_table_device()   )
	{
		...
		// Also allow labelling of busy Logical Volumes whose file system
		// supports labelling while mounted.
		if (selected_partition_ptr->type   == TYPE_PRIMARY &&
		    selected_partition_ptr->status == STAT_REAL    &&
		    selected_filesystem.busy                       &&
...
void Win_GParted::on_show()
{
	...
	//
	// Same thread registration of callback (this thread is GParted_Core::mainthread
	// where the Glib/Gtk main loop runs the callback).  Can use thread unsafe
	// Glib::signal_idle().connect().
	Glib::signal_idle().connect(sigc::mem_fun(*this, &Win_GParted::initial_device_refresh));
}


// Callback used to load the disk device details for the first time
bool Win_GParted::initial_device_refresh()
{
	menu_gparted_refresh_devices();
	return false;  // One shot, remove this callback.
}
...
void Win_GParted::menu_help_about()
{
	...
	strings .push_back( "Patrick Verner <sales@partedmagic.com>" );
