...
OptionStoreItem::operator Gtk::TreeModel::iterator()
{
...
OptionStoreItem::operator Gtk::TreeModel::const_iterator() const
{
	Gtk::TreeModel::Path path = m_rowreference.get_path();
...
void OptionStoreItemsCollection::insert( const OptionStoreItem& item,
	                                     ...
	                                     bool sensitive )
{
	Gtk::TreeModel::iterator previous_iter = item;
...
{
	// user defined conversion to iter
	Gtk::TreeModel::iterator iter = item;
	m_ref_model ->erase( iter );
}
...
void OptionComboBox::pack_cell_renderers()
{
	...
	pack_start( *manage( new Gtk::CellRendererText() ) );
	add_attribute( *get_cells().back(), "text", OptionStore::Slots::Text );
	add_attribute( *get_cells().back(), "sensitive", OptionStore::Slots::Sensitive );
}
...
{
	return OptionStore::Item( util_get_optionstore(), get_active() );
}
