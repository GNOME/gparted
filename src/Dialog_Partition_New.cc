                                     ...
                                     const std::vector<FS> & FILESYSTEMS )
{
	...
	combo_filesystem .set_active_row( first_creatable_fs ) ;
...
const Partition & Dialog_Partition_New::Get_New_Partition()
{
	...
	new_partition->Set( device_path,
	                    ...
	                    new_count, part_type,
	                    FILESYSTEMS[combo_filesystem.get_active_row_index()].filesystem,
	                    new_start, new_end,
	                    sector_size,
	...
	//set alignment
	switch ( combo_alignment .get_active_row_index() )
	{
		case 0:
...
void Dialog_Partition_New::combobox_changed( bool type )
{
	...
	if ( type )
	{
		if ( combo_type .get_active_row_index() == GParted::TYPE_EXTENDED &&
		     combo_filesystem .get_n_rows() < FILESYSTEMS .size() )
		{
			combo_filesystem .append( Utils::get_filesystem_string( GParted::FS_EXTENDED ) ) ;
			combo_filesystem .set_active_row( combo_filesystem .get_n_rows() -1 ) ;
			combo_filesystem .set_sensitive( false ) ;
		}
		else if ( combo_type .get_active_row_index() != GParted::TYPE_EXTENDED &&
			  combo_filesystem .get_n_rows() == FILESYSTEMS .size() )
		{
			combo_filesystem .erase( combo_filesystem .get_n_rows() -1 ) ;
	...
	if ( ! type )
	{
		fs = FILESYSTEMS[ combo_filesystem .get_active_row_index() ] ;
...
void Dialog_Partition_New::Build_Filesystems_Combo( bool only_unformatted ) 
{
	...
	for ( unsigned int t = 0 ; t < FILESYSTEMS .size( ) ; t++ ) 
	{
		...
		if( FILESYSTEMS[ t ] .filesystem == GParted::FS_EXTENDED )
			continue ;
		combo_filesystem .append( Utils::get_filesystem_string( FILESYSTEMS[ t ] .filesystem ) ) ;
		combo_filesystem .set_row_sensitive( combo_filesystem .get_n_rows() -1,
		...
		if ( ( FILESYSTEMS[ t ] .filesystem == FS_EXT2 ||
		     ...
		     combo_filesystem .get_row_sensitive( combo_filesystem .get_n_rows() -1 )  )
		{
			first_creatable_fs=combo_filesystem .get_n_rows() -1;
			set_first=true;
		}
	}
	...
	if(!set_first)
	{
		...
		for ( unsigned int t = 0 ; t < combo_filesystem .get_n_rows() ; t++ )
			if ( combo_filesystem .get_row_sensitive( t ) )
			{
