...
/* OptionStore_Item_Collection lets you operate on OptionStore model with
   a STL-like container interface. You can act on the sequence of items
   and access through it single items.
   Usually you get an OptionStore_Item_Collection by calling the items()
   method on a OptionStore model. You can also call the items() method on
   a OptionComboBox: it simply redirects the call to the associated
   OptionStore model. */
...
/* OptionStore is the model that backs the data for OptionComboBox.
   ...
   and a boolean slot for row sensitiveness (if it's false the row
   is "grayed out").
   Although you can call any Gtk::ListStore method (from which
   OptionStore inherits) it is usualy convenient to call the items()
...
/* OptionComboBox is a specialized ComboBox that shows a list of
   rows, some of which may be selectively grayed out.
   It is commonly used to display a list of options with some the
   user can choose from and others the user cannot select. */
